...
#path to csv file
csv_file = "../data/output/chat_statistics.csv"
...
def main():
    ...
    #command line prompt reader
    if sys.argv[1] == "prog5-sessionlogger":
        ...
        if sys.argv[2] == "-summary":
            ...
            #print values
            print(f"There are {total_chats} chats to date with user asking {user_utterances} times and system reponding {system_utterances} times. Total duration is {time} seconds.")

        #showchat summary
        elif sys.argv[2] == "-showchat-summary":

            #get the chat number
            chat_numb = int(sys.argv[3])
            ...
            if 0 < chat_numb <= total_chats:
                ...
                #print values
                print(f"Chat {chat_numb} has user asking {user_utterances} times and system responding {system_utterances} times. Total duration is {time} seconds")

            #chat number isnt valid
            else:
                print(f"ERROR: There are {total_chats} sessions. Please choose a valid number.")

        #show chat contents
        elif sys.argv[2] == "-showchat":
