...
def mapToPart(str):
    ...
    if ((str == "QUIT") | (str == "Q") | ("QUIT" in str)):
        ...
        elif (wm.isMatch(str, "STATS") or wm.isMatch(str, "USAGE")):
            text = sl.summary()
            return text
        
        #No company name, user might be asking about usage stats
        elif (wm.isMatch(str, "WHAT INFO") or wm.isMatch(str, "WHAT QUESTIONS")):
            text = "A list of supported questions can be found in doc/test_output.txt"
            print("\n"+text+"\n")
            return text
